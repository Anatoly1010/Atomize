...
path_config_file = os.path.join(path_current_directory, 'config','Agilent_53131a_config.ini')
    ...
    def freq_counter_frequency(self, channel):
        if test_flag != 'test':
            if channel == 'CH1':
                ...
                answer = float(self.device_query(':MEASURE:FREQ? (@3)'))
                return answer
            else:
    ...
    def freq_counter_ratio(self, channel1, channel2):
        if test_flag != 'test':
            if channel1 == 'CH1' and channel2 == 'CH2':
                ...
                answer = float(self.device_query(':MEASure:FREQuency:RATio? (@1),(@3)'))
                return answer
            elif channel1 == 'CH3' and channel2 == 'CH1':
                answer = float(self.device_query(':MEASure:FREQuency:RATio? (@3),(@1)'))
                return answer
            else:
                general.message('Invalid argument')
                sys.exit()
        ...
        if test_flag != 'test':
            if len(frequency) == 2:
                ...
                if ch == 'CH1':
                    ...
                    if temp == 0:
                        answer = float(self.device_query(":FREQuency:EXPected3?"))
                        return answer
                    else:
                        general.message("No expected frequency is found")
                else:
    ...
    def freq_counter_period(self, channel):
        if test_flag != 'test':
            if channel == 'CH1':
                ...
                answer = float(self.device_query(':MEASURE:PERiod? (@3)'))*1000000
                return answer
            else:
                general.message('Invalid argument')
                sys.exit()
            ...
            assert(channel == 'CH1' or channel == 'CH2' or channel == 'CH3'), 'Invalid channel is given'
            answer = test_period
            return answer
