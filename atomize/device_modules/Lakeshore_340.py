...
class Lakeshore_340:
    ...
    def tc_heater_power(self):
        if test_flag != 'test':
            ...
            if int(loop_config) == 1 or int(loop_config) == 2:
                answer = float(self.device_query('HTR? ' + str(loop_config)))
                full_answer = [answer, answer1]
                return full_answer
            elif int(loop_config) == 3 or int(loop_config) == 4:
                answer = float(self.device_query('AOUT? ' + str(loop_config)))
                full_answer = [answer, answer1]
                return full_answer
            else:
                general.message('Invalid loop')
            ...
            if len(sensor) == 1:
                ...
                if loop_config in loop_list:
                    ...
                    if sens in sens_dict:
                        flag = sens_dict[sens]
                        self.device_write('CSET '+ str(loop_config) + ',' + str(flag) +',1,0,0')
                    else:
