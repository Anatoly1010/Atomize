...
class BH_15:
	...
	def magnet_sweep_up(self):
		...
		if (self.swa + self.step_incr <= max_swa):
			...
			if new_swa >= min_swa and new_swa <= max_swa and (new_cf - 0.5*self.sw) >= min_field\
	...
	def fc_start_field(self):
		...
		if self.swa >= min_swa and self.swa <= max_swa and (self.cf - 0.5*self.sw) >= min_field\
	...
	def fc_best_fit_search(self, cf, swa, dire, fac):
		...
		if new_swa >= min_swa and new_swa <= max_swa and (new_cf - 0.5*self.swa_step) >= min_field\
	...
	def fc_set_sw(self, sweep_width):
		...
		while i > 0:
			i -= 1
			self.device_write(f'SW{sweep_width:.2f}')
	...
	def fc_change_field_and_keep_sw(self, field):
		...
		if rem > 0:
			...
			if round(self.swa_step/min_field_step) % round(fc_cf_resolution/min_field_step) == 0:
				...
				if self.swa >= min and self.swa <= max_swa and self.cf - 0.5*self.sw >= \
