...
nonlinear_time_shift = np.unique( general.numpy_round( nonlinear_time_raw, 2 ) ).astype(np.int64)
nonlinear_time_axis = (2 * np.insert(nonlinear_time_shift , 0, 0))[:-1]
# the real number of points after rounding to 2 ns and removing repeated values
POINTS = len( nonlinear_time_axis )
...
def cleanup(*args):
    ...
    pb.pulser_stop()
    file_handler.save_data(file_data, np.c_[nonlinear_time_axis, data_x, data_y], header = header, mode = 'w')
    sys.exit(0)
...
for j in general.scans(SCANS):
    ...
    for i in range(POINTS):
        ...
        process = general.plot_1d(EXP_NAME, nonlinear_time_axis, ( data_x, data_y ), xname = 'Delay',\
        ...
        pb.pulser_redefine_delta_start(name = 'P1', delta_start = str( nonlinear_time_shift[i] ) + ' ns')
        pb.pulser_redefine_delta_start(name = 'P2', delta_start = str( int(2 * nonlinear_time_shift[i]) ) + ' ns')
