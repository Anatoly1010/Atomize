...
while j <= SCANS:
    ...
    for i in range(POINTS):
        ...
        x = 1 + 100*np.exp(-axis_x/ch_time) + 7*np.random.normal(size = (1000))
        y = 1 + 100*np.exp(-axis_x/ch_time) + 7*np.random.normal(size = (1000))
        
        data[0, :, i] = ( data[0, :, i] * (j - 1) + x ) / j
        data[1, :, i] = ( data[0, :, i] * (j - 1) + y ) / j

        general.plot_2d(EXP_NAME, data, start_step = ( (0, 1), (0, STEP) ), xname = 'Time',\
