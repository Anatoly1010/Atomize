...
nonlinear_time_raw = 2 * 10 ** np.linspace( 0, 6.93, POINTS )
...
PULSE_1_START = '0 ns'
PULSE_2_START = '74 ns'
PULSE_3_START = '274 ns'
PULSE_SIGNAL_START = '474 ns'
PHASE_CYCLE = 64
...
pb.pulser_pulse(name = 'P1', channel = 'MW', start = PULSE_2_START, length = PULSE_2_LENGTH, delta_start = str( nonlinear_time[0] ) + ' ns', \
                phase_list = ['+x','+x','+x','+x','-x','-x','-x','-x','+y','+y','+y','+y','-y','-y','-y','-y', \
                              ...
                              '+x','+x','+x','+x','-x','-x','-x','-x','+y','+y','+y','+y','-y','-y','-y','-y', \
                              '+x','+x','+x','+x','-x','-x','-x','-x','+y','+y','+y','+y','-y','-y','-y','-y'])
pb.pulser_pulse(name = 'P2', channel = 'MW', start = PULSE_3_START, length = PULSE_3_LENGTH, delta_start = str( nonlinear_time[0] ) + ' ns', \
                phase_list = ['+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y', \
                              '+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y', \
                              '+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y', \
                              '+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y','+x','-x','+y','-y'])
pb.pulser_pulse(name = 'P3', channel = 'TRIGGER', start = PULSE_SIGNAL_START, length = '100 ns', delta_start = str( nonlinear_time[0] ) + ' ns')
...
header = 'Date: ' + str(datetime.datetime.now().strftime("%d-%m-%Y %H-%M-%S")) + '\n' + \
         'T1 DD Nonlinear\n' + 'Field: ' + str(FIELD) + ' G \n' +  \
          ...
          + 'Horizontal Resolution: ' + str(time_res) + ' ns\n' + 'Vertical Resolution: ' + str( (np.insert(nonlinear_time , 0, 0))[:-1] ) + ' ns\n' \
...
for j in general.scans(SCANS):
    ...
    for i in range(POINTS):
        ...
        pb.pulser_redefine_delta_start(name = 'P2', delta_start = str( nonlinear_time[i] ) + ' ns')
        pb.pulser_redefine_delta_start(name = 'P3', delta_start = str( nonlinear_time[i] ) + ' ns')
        pb.pulser_shift()
