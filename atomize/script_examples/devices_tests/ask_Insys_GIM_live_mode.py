...
pb.pulser_pulse(name = 'P1', channel = 'MW' , start = '320 ns'  , length = '32 ns', phase_list =  ['+x', '-x']) #, phase_list =  ['+x', '-x']
pb.pulser_pulse(name = 'P2', channel = 'MW' , start = '448 ns', length = '32 ns', phase_list =  ['+x', '+x']) # , delta_start='3.2 ns', phase_list =  ['+x', '+x']
...
pb.pulser_repetition_rate('400 Hz')
pb.pulser_open()
...
for j in general.to_infinity():
    ...
    for i in range(PHASES):
        ...
        data[0], data[1] = pb.digitizer_get_curve( POINTS, PHASES, acq_cycle = ['+x', '+x'], live_mode = 1 )
