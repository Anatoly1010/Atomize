...
def cleanup(*args):
    ...
    file_handler.save_data(file_data_2, data_2, header = header_2, mode = 'w')
    sys.exit(0)
...
PULSE_3_LENGTH = '240 ns'
PULSE_4_LENGTH = '240 ns'
PULSE_1_START = '0 ns'
PULSE_2_START = '836 ns'
PULSE_3_START = '1672 ns'
PULSE_4_START = '2608 ns'
PULSE_SIGNAL_START = '4038 ns'
...
cycle_data_x_2 = np.zeros( (PHASE, int(wind)) )
cycle_data_y_2 = np.zeros( (PHASE, int(wind)) )
data_2 = np.zeros( (2, int(wind), POINTS) )
...
awg.awg_pulse(name = 'P2', channel = 'CH0', func = 'WURST', frequency = ('100 MHz', '-330 MHz'), phase = 0, \
...
awg.awg_pulse(name = 'P4', channel = 'CH0', func = 'WURST', frequency = ('100 MHz', '-330 MHz'), phase = 0, \
...
awg.awg_pulse(name = 'P6', channel = 'CH0', func = 'WURST', frequency = ('100 MHz', '-330 MHz'), phase = 0, \
...
pb.pulser_pulse(name = 'P7', channel = 'AWG', start = str( int(PULSE_4_START.split(' ')[0]) + 494 ) + ' ns', length = PULSE_4_LENGTH, \
                delta_start = str(STEP) + ' ns')
awg.awg_pulse(name = 'P8', channel = 'CH0', func = 'WURST', frequency = ('100 MHz', '-330 MHz'), phase = 0, \
...
file_data_2, file_param_2 = file_handler.create_file_parameters('.param')
file_handler.save_header(file_param_2, header = header_2, mode = 'w')
    ...
    while cycle_8 <= CYCLE8:
        ...
        for i in range(POINTS):
            ...
            while k < PHASE:
                ...
                awg.awg_next_phase()
                x_axis, cycle_data_x_2[k], cycle_data_y_2[k] = dig4450.digitizer_get_curve( )

                awg.awg_stop()
                k += 1
            
            x, y = pb.pulser_acquisition_cycle(cycle_data_x_2, cycle_data_y_2, \
                    acq_cycle = ['+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', \
                             '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', \
                             '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', \
                             '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', '+x', '-x', '+x', '-x', '-x', '+x', '-x', '+x', \
            ...
            data_2[0, :, i] = ( data_2[0, :, i] * (cycle_8 - 1 + (j - 1) * CYCLE8 ) + x ) / ( cycle_8 + (j - 1) * CYCLE8 )
            data_2[1, :, i] = ( data_2[1, :, i] * (cycle_8 - 1 + (j - 1) * CYCLE8 ) + y ) / ( cycle_8 + (j - 1) * CYCLE8 )

            process = general.plot_2d(EXP_NAME_2, data_2, start_step = ( (0, time_res), (0, 2 * STEP) ), xname = 'Time',\
                xscale = 'ns', yname = 'Delay', yscale = 'ns', zname = 'Intensity', zscale = 'V', pr = process, \
                text = str(j) + ' / ' + str(cycle_8) + ' / ' + str(i*STEP))
