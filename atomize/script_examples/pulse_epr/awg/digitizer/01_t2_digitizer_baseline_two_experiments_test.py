...
cycle_data_x_2 = np.zeros( (2, int(wind)) )
cycle_data_y_2 = np.zeros( (2, int(wind)) )
...
awg.awg_pulse(name = 'P4', channel = 'CH0', func = 'SINE', frequency = '100 MHz', phase = 0, \
            length = PULSE_1_LENGTH, sigma = PULSE_1_LENGTH, start = PULSE_AWG_1_START, phase_list = ['+x', '-x'])
awg.awg_pulse(name = 'P5', channel = 'CH0', func = 'SINE', frequency = '100 MHz', phase = 0, \
            length = PULSE_2_LENGTH, sigma = PULSE_2_LENGTH, start = PULSE_AWG_2_START, delta_start = str(int(STEP/2)) + ' ns', phase_list = ['+x', '+x'])
...
for j in general.scans(SCANS):
    ...
    for i in range(POINTS):
        ...
        data_2[0, :, i] = ( data_2[0, :, i] * (j - 1) + x ) / j
        data_2[1, :, i] = ( data_2[1, :, i] * (j - 1) + y ) / j

        process = general.plot_2d(EXP_NAME_2, data_2, start_step = ( (0, time_res), (0, STEP) ), xname = 'Time',\
            xscale = 'ns', yname = 'Delay', yscale = 'ns', zname = 'Intensity', zscale = 'V', pr = process, \
            text = 'Scan / Time: ' + str(j) + ' / ' + str(i*STEP))
