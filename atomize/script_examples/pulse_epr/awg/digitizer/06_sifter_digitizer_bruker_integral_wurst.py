...
PULSE_AWG_2_START = '464 ns'
PULSE_AWG_3_START = '928 ns'
PULSE_AWG_4_START = '1700 ns'
...
cycle_data_x = np.zeros( PHASE_STEPS )
cycle_data_y = np.zeros( PHASE_STEPS )
data_x = np.zeros(POINTS)
data_y = np.zeros(POINTS)
x_axis = np.linspace(0, (POINTS - 1)*2*STEP, num = POINTS) 
...
for j in general.scans(SCANS):
    ...
    for i in range(POINTS):
        ...
        while k < 16:
            ...
            prDig = returnThread.rThread(target = dig4450.digitizer_get_curve, args=(), kwargs={'integral': True,})
            prDig.start()
            
            awg.awg_next_phase()
            cycle_data_x[k], cycle_data_y[k]  = prDig.join()
